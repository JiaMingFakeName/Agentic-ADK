...
@Data
@AllArgsConstructor
public class FlowProcessService implements Serializable {
    ...
    public ProcessInstance startFlow(FlowDefinition flowDefinition, Map<String, Object> request, Map<String, Object> response) {
        try {
            ...
            FlowXmlUtils.cacheProcessDefinition(flowDefinition.getDefinitionId(), flowDefinition.getVersion());
            Map<String, Object> requestMap = new HashMap<>();
            requestMap.put(ExecutionConstant.ORIGIN_REQUEST, request.get(ExecutionConstant.ORIGIN_REQUEST));
            requestMap.put(ExecutionConstant.SYSTEM_CONTEXT, request.get(ExecutionConstant.SYSTEM_CONTEXT));
    ...
    public void signal(ProcessInstance processInstance, String activityId, Map<String, Object> request, Map<String, Object> response) {
        log.info("signal, activityId: {}", activityId);
        try {
            PersisterSession.create();
            ...
            FlowXmlUtils.cacheProcessDefinition(processInstance.getProcessDefinitionId(), processInstance.getProcessDefinitionVersion());
            ...
            for (ExecutionInstance executionInstance : executionInstanceList) {
                if (executionInstance.getProcessDefinitionActivityId().equals(activityId)) {
                    ...
                    requestMap.put(ExecutionConstant.ORIGIN_REQUEST, request.get(ExecutionConstant.ORIGIN_REQUEST));
                    requestMap.put(ExecutionConstant.SYSTEM_CONTEXT, request.get(ExecutionConstant.SYSTEM_CONTEXT));
                    requestMap.put(ExecutionConstant.IS_CALLBACK, true);
