...
public class ReferenceFlowNode extends FlowNode {
    ...
    /**
     * 优先使用canvas，如果没有canvas，则使用flowDefinition
     */
    ...
    protected void addProperties(Element serviceTask) {
        ...
        flowDefinitionIdElement.addAttribute("value", flowDefinition.getDefinitionId());

        Element flowVersionElement = properties.addElement("smart:property");
        flowVersionElement.addAttribute("name", "flowVersion");
        flowVersionElement.addAttribute("value", flowDefinition.getVersion());

        Element flowBpmnElement = properties.addElement("smart:property");
        flowBpmnElement.addAttribute("name", "flowBpmn");
        flowBpmnElement.addAttribute("value", flowDefinition.getBpmnXml());
    ...
    protected void generate(Element processElement, String terminationNodeIdIfNextEmpty) {
        ...
        if (canvas != null) {
            FlowDefinition flowDefinition = canvas.deploy();
