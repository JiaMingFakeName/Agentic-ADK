...
public class Runner {
    ...
    public Flowable<Result> run(FlowCanvas canvas, Request request) {
        ...
        log.info("adk flowDefinition: " + flowDefinition);
    ...
    public void signal(TaskInstance taskInstance, Result result) {
        ...
        requestMap.put(ExecutionConstant.ORIGIN_REQUEST, taskInstance.getRequest());
        requestMap.put(ExecutionConstant.SYSTEM_CONTEXT, taskInstance.getSystemContext());
        
        Map<String, Object> resultMap = new HashMap<>();
        resultMap.put(ExecutionConstant.CALLBACK_RESULT, result);
    ...
    public Flowable<Result> run(FlowDefinition flowDefinition, Request request) {
        Map<String, Object> requestMap = new HashMap<>();
        requestMap.put(ExecutionConstant.ORIGIN_REQUEST, request);
        requestMap.put(ExecutionConstant.SYSTEM_CONTEXT, new SystemContext()
                .setInvokeMode(request.getInvokeMode())
                .setRequestParameter(request.getParam()));
        return PipelineUtil.doPipe(PipelineRequest.builder()
                .flowDefinition(flowDefinition)
                .request(requestMap).build());
    }
