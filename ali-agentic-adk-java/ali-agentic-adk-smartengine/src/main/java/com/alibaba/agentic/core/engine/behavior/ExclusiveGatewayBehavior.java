...
public class ExclusiveGatewayBehavior extends AbstractActivityBehavior<ExclusiveGateway> {
    ...
    public void leave(ExecutionContext context, PvmActivity pvmActivity) {
        ...
        flowableResult.doOnNext(result -> {
            ...
            log.warn("GatewayHelper chooseEdge error, exclusiveGateway chooses default edge, systemContext: {}, error is: ", context.getRequest().get(ExecutionConstant.SYSTEM_CONTEXT), throwable);
            GatewayHelper.chooseDefaultEdge(context, pvmActivity);
            return result;
        }).subscribe(result -> {
        }, throwable -> {
            log.error("exclusiveGateway chooses default edge error, throw error.", throwable);
            throw throwable;
        });
    }
}