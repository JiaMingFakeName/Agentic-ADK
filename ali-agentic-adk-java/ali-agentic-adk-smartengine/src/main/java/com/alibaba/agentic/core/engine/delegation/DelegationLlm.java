...
public class DelegationLlm extends FrameworkDelegationBase {
    ...
    @Autowired
    public void registerLlms(Optional<List<BasicLlm>> optionalBasicLlms) {
        optionalBasicLlms.ifPresent(llms -> {
            for (BasicLlm llm : llms) {
                register(llm);
            }
        });
    }
     ...
     * @return
     */
    public static BasicLlm getModel(String model) {
        log.info("adk smart engine DelegationLlm getModel, llmMap: {}, model: {}", JSON.toJSONString(llmMap), model);
...
//        log.info("adk smart engine DelegationLlm execute start, request: {}", executionContext.getRequest());
//        super.execute(executionContext);
//        log.info("adk smart engine DelegationLlm execute finished, request: {}, response: {}", executionContext.getRequest(), executionContext.getResponse());
//    }
    ...
    public Flowable<Result> invoke(SystemContext systemContext, Request request) throws Throwable {
        ...
        log.info("llm invoke, systemContext: {}", systemContext);
        if (InvokeMode.ASYNC.equals(systemContext.getInvokeMode()) && !systemContext.isPause()) {
            try {
                return Flowable.just(basicLlm.asyncInvoke(request.getAsyncTaskId(), llmRequest, systemContext));
            } catch (Throwable throwable) {
                log.error("llm invoke error", throwable);
        ...
        return basicLlm.invoke(llmRequest, systemContext)
                .map(llmResponse -> {
                    ...
                    if (success) {
                        return Result.success(mapper.convertValue(llmResponse, new TypeReference<Map<String, Object>>() {}));
                    }
    ...
    public Map<String, Object> generateRequest(ExecutionContext executionContext, SystemContext systemContext, String activityId) {
        ...
        if (properties.containsKey("messages")) {
            ...
            if (messagesObj instanceof List) {
                request.setMessages((List<Message>) messagesObj);
            } else if (messagesObj instanceof String) {
                request.setMessages(JSONArray.parseArray((String) messagesObj, Message.class));
            }
        }
        if (properties.containsKey("extraParams")) {
            request.setExtraParams(properties.get("extraParams"));
        }
        BasicLlm basicLlm = getModel(request.getModel());
        // 关键：**封装成对象再序列化！**
        JSONObject jsonObject = new ObjectMapper().convertValue(request, JSONObject.class);
        jsonObject.put(PropertyConstant.NODE_SUPPORT_ASYNC, basicLlm.supportAsync());
        return jsonObject;
    }
