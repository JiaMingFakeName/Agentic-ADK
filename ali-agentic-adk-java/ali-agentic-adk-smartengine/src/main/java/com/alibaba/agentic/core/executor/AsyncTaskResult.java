...
public class AsyncTaskResult extends Result {
    ...
    public static AsyncTaskResult success(String id, Map<String, Object> data) {
        return new AsyncTaskResult(true, "200", null, data, id);
    }

    public static AsyncTaskResult fail(String id, Throwable throwable) {
        if (throwable instanceof BaseException) {
            BaseException exception = (BaseException) throwable;
            return new AsyncTaskResult(false, exception.getErrorEnum().getCode(), exception.getMessage(), null, id);
        } else {
            return new AsyncTaskResult(false, ErrorEnum.SYSTEM_ERROR.getCode(), ExceptionUtils.getStackTrace(throwable), null, id);
        }
    }
}
